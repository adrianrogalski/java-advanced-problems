...
import task6.IllegalEmailAddressException;
...
public class EmailAddress {
    ...
    public static EmailAddress of(String address){
        if (!isValidAddress(address)) {
            throw new IllegalEmailAddressException("Niepoprawny format adresu email");
        }
    ...
    public static EmailAddress of(String username, String host){
        if (!isValidUsername(username)) {
            throw new IllegalUsernameException("Niepoprawna nazwa użytkownika");
        }
        if (!isValidHost(host)) {
            throw new IllegalHostException("Niepoprawna domena");
        }
        String newEmail = username + host;
        return new EmailAddress(newEmail);
    }
